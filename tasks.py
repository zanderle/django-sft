...
def coverage(c):
    ...
    """
    c.run("coverage run --source django-sft runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-sft.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_sft")
...
def release(c, bumpsize=''):
    ...
    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_sft.__version__))
    c.run("git push --tags")
